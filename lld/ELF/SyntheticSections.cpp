...
void SFrameSection::buildFde(uint8_t *buf, uint32_t funcSize, uint32_t numFres,
  ...
  buf[offsetof(sframe_func_desc_entry, sfde_func_rep_size)] = repSize;
}
...
void SFrameSection::writeTo(uint8_t *buf) {
  ...
  sortedFdes.reserve(numFdes);

  uint8_t pltHdrFdeBuf[sizeof(sframe_func_desc_entry)];
  SFrameSectionPiece pltHdrSp = {pltHdrFdeBuf,
                                 ctx.target->pltHeaderSFrameFres().data(),
                                 ctx.target->pltHeaderSFrameFres().size()};

  uint8_t pltFdeBuf[sizeof(sframe_func_desc_entry)];
  SFrameSectionPiece pltSp = {pltFdeBuf, ctx.target->pltSFrameFres().data(),
                              ctx.target->pltSFrameFres().size()};
  if (needPlt) {
  ...
  for (const auto [fde, va] : sortedFdes) {
    // TODO: figure out how to report the input section here.
    if (!isInt<32>(va - secAddr)) {
      Err(ctx) << ": sframe PC offset is too large: 0x"
               << Twine::utohexstr(va - secAddr);
      continue;
    }

...
void SFrameSection::addRecords(SFrameInputSection *sec, ArrayRef<RelTy> rels) {
  ...
  for (const auto &[fde, rel] : zip_equal(sec->fdes, rels)) {
    Symbol &S = sec->file->getRelocTargetSym(rel);
    if (!isFdeLive(S)) {
      numFdes--;
      fde.live = false;
      fde.fdeOutputOff = uint64_t(-1);
      numFres -=
          read32(ctx, sec->content().data() + fde.fdeInputOff +
...
void SFrameSection::finalizeContents() {
  ...
  if (ctx.in.plt->isNeeded()) {
    // There will be two fdes: One for the header of type PCINC and one for all
    // the rest of the entries of type PCMASK. Both have their own set of
    // fres. For now we just need to reserve space.
    // needPlt = true;
    // outputFdes.push_back({0, 0});
    // outputFdes.push_back({0, 0});
    // numFres += 4; // FIXME: Row count should be retrieved from the target.
    // freSubSecLen += ctx.target->pltHeaderSFrameFres().size();
    // freSubSecLen += ctx.target->pltSFrameFres().size();
  }
  size = fdeSubSecOff() + fdeSubSecLen() + freSubSecLen;
}

void SFrameSection::assignOutputLocations() {
  // There is a circularity problem here: We can't relocate the pc relative
  // sfde_func_start_address until we know the fde's output location. But the
  // subsection should be sorted on the start address, so we can't assign the
  // output location until we know that address. Unlike an eh_frame_hdr, we
  // don't have an indirection into the eh_frame section itself, so both have to
  ...
  // relocations. During relocation scanning, sframe relocations assigned
  // incorrect ouput addresses. Assign fde output locations here, and cleanup
  // the relocations output addresses.
  using VaFde = std::tuple<SFrameSectionPiece &, Relocation&>;
  SmallVector<VaFde> fdesByAddr;
  for (SFrameInputSection *sec : sections) {
    for (const auto &[fde, rel] : zip_equal(sec->fdes, sec->relocations)) {
      if (fde.live)
        fdesByAddr.push_back({fde, rel});
    }
  }
  llvm::stable_sort(fdesByAddr, [&](const VaFde &l, const VaFde &r) {
    const Relocation& lrel = std::get<1>(l);
    const Relocation& rrel = std::get<1>(r);
    return lrel.sym->getVA(ctx) + lrel.addend <
           rrel.sym->getVA(ctx) + rrel.addend;
  });

  size_t fdeOff = fdeSubSecOff();
  for (auto &[fde, rel] : fdesByAddr) {
    fde.fdeOutputOff = fdeOff;
    // sfde_func_start_address is the first field in the entry.
    rel.offset = fdeOff;
    fdeOff += sizeof(llvm::sframe::sframe_func_desc_entry);
  }
}
...
void SFrameSection::writeTo(uint8_t *buf) {
  ...
  uint32_t freOutSecOff = fdeSubSecOff() + fdeSubSecLen();
  write32(ctx, buf + offsetof(sframe_header, sfh_freoff), freOutSecOff);

  uint64_t freOutOff = 0;
  for (SFrameInputSection *sec : sections) {
    for (auto fde : sec->fdes) {
      if (!fde.live)
        continue;
      memcpy(buf + fde.fdeOutputOff, sec->content().data() + fde.fdeInputOff,
             sizeof(sframe_func_desc_entry));
      write32(ctx,
              buf + fde.fdeOutputOff +
                  offsetof(sframe_func_desc_entry, sfde_func_start_fre_off),
              freOutOff);
      memcpy(buf + freOutSecOff + freOutOff,
             sec->content().data() + fde.freInputOff, fde.freSize);
  ...
  bool GotSection::isNeeded() const {
    // Needed if the GOT symbol is used or the number of entries is more than
    // just the header. A GOT with just the header may not be needed.
  ...
  void GotSection::writeTo(uint8_t *buf) {
    ...
    for (const AuthEntryInfo &authEntry : authEntries) {
      ...
      //   Signed GOT entries use the IA key for symbols of type STT_FUNC and
      //   the DA key for all other symbol types, with the address of the GOT
      //   entry as the modifier. The static linker must encode the signing
  ...
  void MipsGotSection::addEntry(InputFile & file, Symbol & sym, int64_t addend,
    ...
    if (expr == RE_MIPS_GOT_LOCAL_PAGE) {
      if (const OutputSection *os = sym.getOutputSection())
        ...
        g.local16.insert(
            {{nullptr, getMipsPageAddr(sym.getVA(ctx, addend))}, 0});
    } else if (sym.isTls())
      g.tls.insert({&sym, 0});
  ...
  size_t MipsGotSection::FileGot::getEntriesNum() const {
    return getPageEntriesNum() + local16.size() + global.size() +
           relocs.size() + tls.size() + dynTlsSymbols.size() * 2;
  }

  size_t MipsGotSection::FileGot::getPageEntriesNum() const {
    size_t num = 0;
    for (const std::pair<const OutputSection *, FileGot::PageBlock> &p :
         pagesMap)
      num += p.second.count;
    return num;
  }
  ...
  uint64_t MipsGotSection::getPageEntryOffset(
      const InputFile *f, const Symbol &sym, int64_t addend) const {
  ...
  uint64_t MipsGotSection::getSymEntryOffset(
      const InputFile *f, const Symbol &s, int64_t addend) const {
    const FileGot &g = gots[f->mipsGotIndex];
  ...
  void MipsGotSection::build() {
    ...
    for (FileGot &srcGot : gots) {
      ...
      if (tryMergeGots(mergedGots.front(), srcGot, true)) {
        ...
        // inputs are just right, we could allow the primary GOT to become 1 or
        // 2 words bigger due to ignoring the header size.
      ...
      for (std::pair<Symbol *, size_t> &p : got.dynTlsSymbols) {
        ...
        if (s == nullptr) {
          ...
          ctx.mainPart->relaDyn->addSymbolReloc(ctx.target->tlsModuleIndexRel,
                                                *this, offset, *s);
          // However, we can skip writing the TLS offset reloc for
          // non-preemptible symbols since it is known even in shared libraries
          if (!s->isPreemptible)
            continue;
  ...
  uint64_t MipsGotSection::getGp(const InputFile *f) const {
    ...
    return getVA() + gots[f->mipsGotIndex].startIndex * ctx.arg.wordsize +
           0x7ff0;
  }
  ...
  void MipsGotSection::writeTo(uint8_t *buf) {
    ...
    for (const FileGot &g : gots) {
      ...
      for (const std::pair<const OutputSection *, FileGot::PageBlock> &l :
        ...
        for (size_t pi = 0; pi < pageCount; ++pi)
          write(l.second.firstIndex + pi, nullptr,
      ...
      for (const std::pair<Symbol *, size_t> &p : g.dynTlsSymbols) {
        if (p.first == nullptr && !ctx.arg.shared)
          ...
          // write anything to the GOT here. When using Elf_Rel relocations the
          // value one will be treated as an addend and will cause crashes at
          // runtime
          if (!ctx.arg.shared)
            write(p.second, nullptr, 1);
  ...
  // section. I don't know why we have a BSS style type for the section but it
  // is consistent across both 64-bit PowerPC ABIs as well as the 32-bit PowerPC
  // ABI.
  ...
  bool GotPltSection::isNeeded() const {
    // We need to emit GOTPLT even if it's empty if there's a relocation
    // relative to it.
  ...
  IgotPltSection::IgotPltSection(Ctx & ctx)
      : SyntheticSection(ctx, getIgotPltName(ctx),
                         ctx.arg.emachine == EM_PPC64 ? SHT_NOBITS
                                                      : SHT_PROGBITS,
  ...
  StringTableSection::StringTableSection(Ctx & ctx, StringRef name,
                                         bool dynamic)
      : SyntheticSection(ctx, name, SHT_STRTAB,
                         dynamic ? (uint64_t)SHF_ALLOC : 0, 1),
        dynamic(dynamic) {
  ...
  // for duplicates. It is optional because the name of global symbols are
  // already uniqued and hashing them again has a big cost for a small value:
  // uniquing them with some other string that happens to be the same.
  ...
  //
  // - part.relaDyn
  // - ctx.in.relaPlt: this is included if a linker script places .rela.plt
  // inside
  //   .rela.dyn
  //
  ...
  DynamicSection<ELFT>::computeContents() {
    ...
    // DT_DEBUG is a pointer to debug information used by debuggers at runtime.
    // We need it for each process, so we don't write it for DSOs. The loader
    // writes the pointer into this entry.
    //
    // DT_DEBUG is the only .dynamic entry that needs to be written to. Some
    // systems (currently only Fuchsia OS) provide other means to give the
    // debugger this information. Such systems may choose make .dynamic
    // read-only. If the target is such a system (used -z rodynamic) don't write
    // DT_DEBUG.
    ...
    if (ctx.arg.emachine == EM_MIPS) {
      ...
      if (ctx.in.mipsRldMap) {
        ...
        addInt(DT_MIPS_RLD_MAP_REL, ctx.in.mipsRldMap->getVA() -
                                        (getVA() + entries.size() * entsize));
      }
    }

    // DT_PPC_GOT indicates to glibc Secure PLT is used. If DT_PPC_GOT is
    // absent, glibc assumes the old-style BSS PLT layout which we don't
    // support.
    if (ctx.arg.emachine == EM_PPC)
      addInSec(DT_PPC_GOT, *ctx.in.got);

    // Glink dynamic tag is required by the V2 abi if the plt section isn't
    // empty.
    if (ctx.arg.emachine == EM_PPC64 && ctx.in.plt->isNeeded()) {
      // The Glink tag points to 32 bytes before the first lazy symbol
      // resolution stub, which starts directly after the header.
  ...
  RelocationBaseSection::RelocationBaseSection(
      Ctx & ctx, StringRef name, uint32_t type, int32_t dynamicTag,
      int32_t sizeDynamicTag, bool combreloc, unsigned concurrency)
  ...
  void RelocationBaseSection::addSymbolReloc(
      RelType dynType, InputSectionBase & isec, uint64_t offsetInSec,
  ...
  RelocationSection<ELFT>::RelocationSection(
      Ctx & ctx, StringRef name, bool combreloc, unsigned concurrency)
  ...
  bool AndroidPackedRelocationSection<ELFT>::updateAllocSize(Ctx & ctx) {
    // This function computes the contents of an Android-format packed
    // relocation section.
    //
    // This format compresses relocations by using relocation groups to factor
    // out fields that are common between relocations and storing deltas from
    // previous relocations in SLEB128 format (which has a short representation
    // for small numbers). A good example of a relocation type with common
    // fields is R_*_RELATIVE, which is normally used to represent function
    // pointers in vtables. In the REL format, each relative relocation has the
    // same r_info field, and is only different from other relative relocations
    // in terms of the r_offset field. By sorting relocations by offset,
    // grouping them by r_info and representing each relocation with only the
    // delta from the previous offset, each 8-byte relocation can be compressed
    // to as little as 1 byte (or less with run-length encoding). This
    // relocation packer was able to reduce the size of the relocation section
    // in an Android Chromium DSO from 2,911,184 bytes to 174,693 bytes, or 6%
    ...
    // elements are the total number of relocations in the section and an
    // initial r_offset value. The remaining elements define a sequence of
    // relocation groups. Each relocation group starts with a header consisting
    ...
    //      that the runtime linker can speed-up symbol lookup by implementing
    //      an 1-entry cache.
    ...
    // Group relocations with the same r_info. Note that each group emits a
    // group header and that may make the relocation section larger. It is hard
    // to estimate the size of a group header as the encoded size of that varies
    ...
    // we can usually get a smaller relocation section if we group relocations
    // with 0 addend as well.
    ...
    // Returns whether the section size changed. We need to keep recomputing
    // both section layout and the contents of this section until the size
    // converges because changing this section's size can affect section layout,
    // which in turn can affect the sizes of the LEB-encoded integers stored in
    // this section.
  ...
  template <class ELFT> bool RelrSection<ELFT>::updateAllocSize(Ctx & ctx) {
    ...
    // address entry encodes 1 relocation. The subsequent bitmap entries encode
    // up to 63 relocations each, at subsequent offsets following the last
    // address entry.
    //
    // The bitmap entries must have 1 in the least significant bit. The
    // assumption here is that an address cannot have 1 in lsb. Odd addresses
    // are not supported.
    //
    ...
    // significant bit represents the machine word immediately following the
    // initial address, and each bit that follows represents the next word, in
    // linear order. As such, a single bitmap can encode up to 31 relocations in
    // a 32-bit object, and 63 relocations in a 64-bit object.
    //
  ...
  // ftp://www.linux-mips.org/pub/linux/mips/doc/ABI/mipsabi.pdf
  static void sortMipsSymbols(Ctx & ctx,
                              SmallVector<SymbolTableEntry, 0> & syms) {
    llvm::stable_sort(
        syms, [&](const SymbolTableEntry &l, const SymbolTableEntry &r) {
  ...
  // sort symbol entries in this function. (For .dynsym, we don't do that
  // because symbols for dynamic linking are inherently all globals.)
  //
  // Aside from above, we put local symbols in groups starting with the STT_FILE
  // symbol. That is convenient for purpose of identifying where are local
  // symbols coming from.
  void SymbolTableBaseSection::sortSymTabSymbols() {
    ...
    // happens because STT_FILE is always the first symbol in the object and
    ...
    for (SymbolTableEntry &ent : symbols) {
      ...
      if (BssSection *commonSec = getCommonSec(relocatable, sym)) {
        ...
        if (isDefinedHere) {
          ...
          // significant for undefined symbols, so whether copying it or not is
          // up to us if that's the case. We'll leave it as zero because by not
  ...
  void GnuHashTableSection::writeTo(uint8_t *buf) {
    ...
    for (const Entry &sym : symbols) {
      // When C = 64, we choose a word with bits [6:...] and set 1 to two bits
      // in the word using bits [0:5] and [26:31].
  ...
  void GnuHashTableSection::addSymbols(SmallVectorImpl<SymbolTableEntry> & v) {
    ...
    auto mid = std::stable_partition(
        v.begin(), v.end(), [&](const SymbolTableEntry &s) {
  ...
  // enhancement feature called CET is enabled. In this comment, I'll explain
  // what the feature is and why we have two PLT sections if CET is enabled.
  //
  ...
  // "landing pad" instruction (which is actually a repurposed NOP instruction
  // and now called "endbr32" or "endbr64") is at the jump target. If the jump
  // target does not start with that instruction, the processor raises an
  // exception instead of continuing executing code.
  //
  ...
  // of a function that is not supporsed to be a indirect jump target,
  // preventing certain types of attacks such as ROP or JOP.
  //
  // Note that the processors in the market as of 2019 don't actually support
  // the feature. Only the spec is available at the moment.
  //
  ...
  // start with endbr. The problem is there's no extra space for endbr (which is
  // 4 bytes long), as the PLT entry is only 16 bytes long and all bytes are
  // already used.
  //
  ...
  // .got.plt AND code to resolve a dynamic symbol lazily. With the 2-PLT
  // scheme, the former code is written to .plt.sec, and the latter code is
  // written to .plt.
  //
  ...
  // with a resolved address, so that the subsequent function calls directly
  // jump to a desired location from .plt.sec.
  //
  ...
  RelroPaddingSection::RelroPaddingSection(Ctx & ctx)
      : SyntheticSection(ctx, ".relro_padding", SHT_NOBITS,
                         SHF_ALLOC | SHF_WRITE, 1) {}
  ...
  static Expected<DebugNamesBaseSection::IndexEntry *> readEntry(
    ...
    if (it == ni.getAbbrevs().end())
      return createStringError(
          ...
          "abbrev code not found in abbrev table: %" PRIu32, ie->abbrevCode);
  ...
  void DebugNamesBaseSection::parseDebugNames(
    ...
    for (const DWARFDebugNames::NameIndex &ni : *inputChunk.llvmDebugNames) {
      ...
      // Assign parent pointers, which will be used to update DW_IDX_parent
      // index attributes. Note: offsetMap[0] does not exist, so parentOffset ==
      // 0 will get parentEntry == null as well.
    ...
    for (InputChunk &inputChunk : inputChunks) {
      for (auto [i, ni] : enumerate(*inputChunk.llvmDebugNames)) {
        for (const DWARFDebugNames::Abbrev &oldAbbrev : ni.getAbbrevs()) {
          ...
          // Profile the abbrev, get or assign a new code, then record the
          // abbrev code mapping.
          FoldingSetNodeID id;
          abbrev.Profile(id);
          uint32_t newCode;
        ...
        for (auto j : seq(inputChunk.nameData.size())) {
          ...
          for (NameEntry &ne : nd.nameEntries) {
            ...
            if (inserted)
              ...
              nameVec[it->second].indexEntries.append(
                  std::move(ne.indexEntries));
          }
        }
      }
    });
    ...
    parallelFor(0, numShards, [&](size_t shard) {
      for (NameEntry &ne : nameVecs[shard]) {
        for (IndexEntry &ie : ne.entries()) {
          ...
          for (const auto &[a, v] :
               zip_equal(abbrev->attributes, ie.attrValues))
  ...
  void DebugNamesBaseSection::init(
    ...
    // Collect and remove input .debug_names sections. Save InputSection
    // pointers to relocate string offsets in `writeTo`.
    ...
    // Compute section header (except unit_length), abbrev table, and entry
    // pool.
  ...
  template <class ELFT> void DebugNamesSection<ELFT>::finalizeContents() {
    ...
    // Relocate string offsets in the name table with .debug_str + X
    // relocations.
          ...
          for (AttrValue value : ie.attrValues) {
            ...
            case 1:
              *buf++ = value.attrValue;
              break;
            case 2:
              endian::writeNext<uint16_t, ELFT::Endianness>(buf,
                                                            value.attrValue);
              break;
            case 4:
              endian::writeNext<uint32_t, ELFT::Endianness>(buf,
  ...
  static SmallVector<GdbIndexSection::CuEntry, 0> readCuList(DWARFContext &
                                                             dwarf) {
  ...
  static SmallVector<GdbIndexSection::AddressEntry, 0> readAddressAreas(
      Ctx & ctx, DWARFContext & dwarf, InputSection * sec) {
  ...
  template <class ELFT>
  static SmallVector<GdbIndexSection::NameAttrEntry, 0> readPubNamesAndTypes(
      Ctx & ctx, const LLDDwarfObj<ELFT> &obj,
    ...
    for (const LLDDWARFSection *pub : {&pubNames, &pubTypes}) {
      ...
      for (const DWARFDebugPubTable::Set &set : table.getData()) {
        // The value written into the constant pool is kind << 24 | cuIndex. As
        // we don't know how many compilation units precede this object to
        // compute cuIndex, we compute (kind << 24 | cuIndexInThisObject)
        // instead, and add the number of preceding compilation units later.
  ...
  std::unique_ptr<GdbIndexSection> GdbIndexSection::create(Ctx & ctx) {
    ...
    // LLDDwarfObj<ELFT>::LLDDwarfObj. If we do lightweight parsing in the
    // future, note that isec->data() may uncompress the full content, which
    ...
    parallelFor(0, files.size(), [&](size_t i) {
      ...
      // If the are multiple compile units .debug_info (very rare ld -r
      // --unique), this only picks the last one. Other address ranges are lost.
  ...
  void EhFrameHeader::writeTo(uint8_t *buf) {
    ...
    // of EhFrameHeader depend on the relocated contents of EhFrameSection and
    // we don't know which order the sections will be written in.
  }
  ...
  void VersionDefinitionSection::finalizeContents() {
    ...
    // sh_info should be set to the number of definitions. This fact is missed
    // in documentation, but confirmed by binutils community:
  ...
  void VersionTableSection::writeTo(uint8_t *buf) {
    ...
    for (const SymbolTableEntry &s :
         getPartition(ctx).dynSymTab->getSymbols()) {
  ...
  bool ARMExidxSyntheticSection::addSection(InputSection * isec) {
    ...
    // and we would have to erase at a late stage relocations from merged
    // entries. Given that exception tables are already position independent and
    // a binary analyzer could derive the relocations we choose to erase the
    // relocations.
  ...
  // The .ARM.exidx table must be sorted in ascending order of the address of
  // the functions the table describes. std::optionally duplicate adjacent table
  ...
  void ARMExidxSyntheticSection::finalizeContents() {
    // Ensure that any fixed-point iterations after the first see the original
    // set of sections.
    ...
    // processSectionCommands() and ICF. A /DISCARD/ entry in SECTIONS command
    // or ICF may remove executable InputSections and their dependent .ARM.exidx
  ...
  // To write the .ARM.exidx table from the ExecutableSections we have three
  // cases 1.) The InputSection has a .ARM.exidx InputSection in its dependent
  // sections.
  ...
  void PPC32Got2Section::finalizeContents() {
    ...
    // PPC32PltCallStub::writeTo(). The purpose of this empty synthetic section
    // is to collect input sections named ".got2".
    for (SectionCommand *cmd : getParent()->commands)
      if (auto *isd = dyn_cast<InputSectionDescription>(cmd)) {
        for (InputSection *isec : isd->sections) {
  ...
  std::optional<uint32_t> PPC64LongBranchTargetSection::addEntry(
      const Symbol *sym, int64_t addend) {
    auto res =
  ...
  void PPC64LongBranchTargetSection::writeTo(uint8_t *buf) {
    // If linking non-pic we have the final addresses of the targets and they
    // get written to the table directly. For pic the dynamic linker will
    // allocate the section and fill it.
    if (ctx.arg.isPic)
      return;
  ...
  bool PPC64LongBranchTargetSection::isNeeded() const {
    ...
    // creation. Because of this, if we don't create any long-branch thunks we
    // end up with an empty .branch_lt section in the binary.
  ...
  template <typename ELFT>
  PartitionProgramHeadersSection<ELFT>::PartitionProgramHeadersSection(Ctx &
                                                                       ctx)
  ...
  void PartitionIndexSection::writeTo(uint8_t *buf) {
    ...
    for (size_t i = 1; i != ctx.partitions.size(); ++i) {
      ...
      write32(ctx, buf + 8,
              next->getVA() - ctx.partitions[i].elfHeader->getVA());
  ...
  // Fully static executables don't support MTE globals at this point in time,
  // as we currently rely on:
  ...
  void MemtagAndroidNote::writeTo(uint8_t *buf) {
    static_assert(sizeof(kMemtagAndroidNoteName) == 8,
                  "Android 11 & 12 have an ABI that the note name is 8 bytes "
                  "long. Keep it "
  ...
  static size_t createMemtagGlobalDescriptors(
      Ctx & ctx, const SmallVector<const Symbol *, 0> &symbols,
      uint8_t *buf = nullptr) {
    ...
    for (const Symbol *sym : symbols) {
      ...
      const uint64_t stepToEncode =
          ((addr - lastGlobalEnd) / kMemtagGranuleSize) << kMemtagStepSizeBits;
      if (sizeToEncode < (1 << kMemtagStepSizeBits)) {
        sectionSize += computeOrWriteULEB128(stepToEncode | sizeToEncode, buf,
                                             sectionSize);
      } else {
        sectionSize += computeOrWriteULEB128(stepToEncode, buf, sectionSize);
        sectionSize +=
            computeOrWriteULEB128(sizeToEncode - 1, buf, sectionSize);
      }
  ...
  static Defined *addOptionalRegular(Ctx & ctx, StringRef name,
  ...
  template <class ELFT> void elf::createSyntheticSections(Ctx & ctx) {
    ...
    if (ctx.arg.strip != StripPolicy::All) {
      ctx.in.strTab =
          std::make_unique<StringTableSection>(ctx, ".strtab", false);
      ctx.in.symTab =
    ...
    if (ctx.arg.zRelro && ((ctx.script->phdrsCommands.empty() &&
    ...
    // _GLOBAL_OFFSET_TABLE_ is defined relative to either .got.plt or .got.
    // Treat it as a relocation and ensure the referenced section is created.
